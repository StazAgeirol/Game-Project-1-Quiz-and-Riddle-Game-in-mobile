...
public class GameplayLevel6 : MonoBehaviour
{
    ...
    void Start() // ===== function Main Starting Program
    {
        ...
        Questions.Add(new QuestionTemplate(1, "Flat as a leaf, round as a ring, Has two eyes, can't see a thing.  What is it?", "BUTTON", "B"));
        Questions.Add(new QuestionTemplate(2, "The more you take, the more you leave behind. What am I?", "FOOTSTEP", "F"));
        Questions.Add(new QuestionTemplate(3, "David's father has three sons : Snap, Crackle and _____ ?", "DAVID", "D"));
        Questions.Add(new QuestionTemplate(4, "Two friends stand and travel together, one nearly useless without the other", "BOOTS", "B"));
        Questions.Add(new QuestionTemplate(5, "It is a journey whose path depends, on an other’s vision of where it ends. ", "BOOK", "B"));
        Questions.Add(new QuestionTemplate(6, "Up and down they go and travel, but never do they move an inch", "STAIRS", "S"));
        NextButtonClick();
    ...
    void Update() // ===== function update every frame
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleScore_6 = ScoreFieldText.text = "" + Score;
            ...
            if (ramdomNumber == i)
            {
                if (onClick.currentword.Length == Questions[i].CorrectAnswer.Length)
                {
                    if (onClick.currentword == Questions[i].CorrectAnswer)
                    {
                        ...
                        if (QuestionCount == TotalQuestionPerRound)
                        {
							playersaveSystem.GetComponent<PlayerSaveSystem>().Riddlelevel6 = true;
                            NextButton.gameObject.SetActive(false);
                            Time.timeScale = 1f;
                            ProgressButton.gameObject.SetActive(true);
                            timeofgamestop = false;
    ...
    public void UpdateTimerUI() // ===== function the timer will show up overall answer after finish answering the question or go it wrong answered
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleTime_6 = Timeofgame.text = hourCount + "h:" + minuteCount + "m:" + (int)secondsCount + "s";
        if (secondsCount >= 60)
        {
            minuteCount++;
            secondsCount = 0;
