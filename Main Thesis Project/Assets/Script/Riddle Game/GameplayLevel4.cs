...
public class GameplayLevel4 : MonoBehaviour
{
    ...
    void Start() // ===== function Main Starting Program
    {
        ...
        Questions.Add(new QuestionTemplate(0, "What is shaped like a box, has no feet, and runs up and down?", "ELEVATOR", "E"));
        Questions.Add(new QuestionTemplate(1, "What travels around the world but stays in one spot?", "STAMP", "S"));
        Questions.Add(new QuestionTemplate(2, "Men desire me in public, but fear me in private", "TRUTH", "T"));
        Questions.Add(new QuestionTemplate(3, "What has a head and a tail, can flip but has no legs?", "COIN", "C"));
        Questions.Add(new QuestionTemplate(4, "What tastes better than it smells?", "TONGUE", "T"));
        Questions.Add(new QuestionTemplate(5, "Say my name and I disappear. What am I?", "SILENCE", "S"));
        NextButtonClick();
    ...
    {
        SceneManager.LoadScene("GameplayLevel4");
    ...
    void Update() // ===== function update every frame
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleScore_4 = ScoreFieldText.text = "" + Score;
            ...
            if (ramdomNumber == i)
            {
                if (onClick.currentword.Length == Questions[i].CorrectAnswer.Length)
                {
                    if (onClick.currentword == Questions[i].CorrectAnswer)
                    {
                        ...
                        if (QuestionCount == TotalQuestionPerRound)
                        {
							playersaveSystem.GetComponent<PlayerSaveSystem>().Riddlelevel4 = true;
                            NextButton.gameObject.SetActive(false);
                            Time.timeScale = 1f;
                            ProgressButton.gameObject.SetActive(true);
                            timeofgamestop = false;
    ...
    public void UpdateTimerUI() // ===== function the timer will show up overall answer after finish answering the question or go it wrong answered
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleTime_4 = Timeofgame.text = hourCount + "h:" + minuteCount + "m:" + (int)secondsCount + "s";
        if (secondsCount >= 60)
        {
            minuteCount++;
            secondsCount = 0;
