...
public class GameplayLevel8 : MonoBehaviour
{
    public ParticleSystem Effect_01, Effect_02;
    public ParticleSystem Effect_background;
    public Transform EffectsPrefabContainer;
    public Animator animator, SAchieveanimator, messagepop, AnsrBtnPrefab_anim, Level_anim;
    ...
    void Start() // ===== function Main Starting Program
    {
        ...
        Questions.Add(new QuestionTemplate(1, "I have hundreds of legs but I can only lean;  You make me feel dirty so you feel clean.  What am I?", "BROOM", "B"));
        Questions.Add(new QuestionTemplate(2, "If you have me, you want to share me. If you share me, you haven't got me. What am I?", "SECRETE", "S"));
        Questions.Add(new QuestionTemplate(3, "Men seize it from its home, tear apart its flesh, drink the sweet blood, then cast its skin aside.", "ORANGE", "O"));
        Questions.Add(new QuestionTemplate(4, "At night they come without being fetched. By day they are lost without being stolen. What are they?", "STARS", "S"));
        Questions.Add(new QuestionTemplate(5, "Alive without breath, As cold as death, Clad in mail never clinking, Never thirsty, ever drinking", "FISH", "F"));
        Questions.Add(new QuestionTemplate(6, "I have keys without key locks. I have space without rooms. You can enter but you cannot go outside.", "KEYBOARD", "K"));
        NextButtonClick();
    ...
    {
        SceneManager.LoadScene("GameplayLevel8");
    ...
    void Update() // ===== function update every frame
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleScore_8 = ScoreFieldText.text = "" + Score;
            ...
            if (ramdomNumber == i)
            {
                if (onClick.currentword.Length == Questions[i].CorrectAnswer.Length)
                {
                    if (onClick.currentword == Questions[i].CorrectAnswer)
                    {
                        ...
                        if (QuestionCount == TotalQuestionPerRound)
                        {
							playersaveSystem.GetComponent<PlayerSaveSystem>().Riddlelevel8 = true;
                            NextButton.gameObject.SetActive(false);
                            Time.timeScale = 1f;
                            ProgressButton.gameObject.SetActive(true);
                            timeofgamestop = false;
    ...
    public void UpdateTimerUI() // ===== function the timer will show up overall answer after finish answering the question or go it wrong answered
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleTime_8 = Timeofgame.text = hourCount + "h:" + minuteCount + "m:" + (int)secondsCount + "s";
        if (secondsCount >= 60)
        {
            minuteCount++;
            secondsCount = 0;
