...
public class GameplayLevel1 : MonoBehaviour
{
    public static GameplayLevel1 gp1;
    ...
    void Start() // ===== function Main Starting Program
    {
        ...
        Questions.Add(new QuestionTemplate(1, "What are moving left to right, right now?", "EYES", "E"));
        Questions.Add(new QuestionTemplate(2, "What's a lifeguard's favorite game?", "POOL", "P"));
        Questions.Add(new QuestionTemplate(3, "I have a head and a tail but no body... What am I?", "COIN", "C"));
        Questions.Add(new QuestionTemplate(4, "What food lives at the beach?", "SANDWICH", "S"));
        Questions.Add(new QuestionTemplate(5, "Brought to the table. Cut and served. Never eaten", "CARD", "C"));
        Questions.Add(new QuestionTemplate(6, "You heard me before, yet you hear me again, Then I die, till you call me again.", "ECHO", "E")); ;
        NextButtonClick();
    ...
    void Update() // ===== function update every frame
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleScore_1 = ScoreFieldText.text = "" + Score;
            ...
            if (ramdomNumber == i)
            {
                if (onClick.currentword.Length == Questions[i].CorrectAnswer.Length)
                {
                    if (onClick.currentword == Questions[i].CorrectAnswer)
                    {
                        ...
                        if (QuestionCount == TotalQuestionPerRound)
                        {
							playersaveSystem.GetComponent<PlayerSaveSystem>().Riddlelevel1 = true;
                            NextButton.gameObject.SetActive(false);
                            Time.timeScale = 1f;
                            ProgressButton.gameObject.SetActive(true);
                            timeofgamestop = false;
    ...
    public void UpdateTimerUI() // ===== function the timer will show up overall answer after finish answering the question or go it wrong answered
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleTime_1 = Timeofgame.text = hourCount + "h:" + minuteCount + "m:" + (int)secondsCount + "s";
        if (secondsCount >= 60)
        {
            minuteCount++;
            secondsCount = 0;
