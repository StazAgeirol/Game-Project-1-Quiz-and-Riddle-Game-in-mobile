...
public class GameplayLevel5 : MonoBehaviour
{
    ...
    void Start() // ===== function Main Starting Program
    {
        ...
        Questions.Add(new QuestionTemplate(1, "I am the breath of relief and depression. What am I?", "SIGH", "S"));
        Questions.Add(new QuestionTemplate(2, "What belongs to you, but other people use it more than you?", "NAME", "N"));
        Questions.Add(new QuestionTemplate(3, "What five long word become shorter when you add two letters", "SHORT", "S"));
        Questions.Add(new QuestionTemplate(4, "What is so fragile, even speaking its name will break it?", "SILENCE", "S"));
        Questions.Add(new QuestionTemplate(5, "The more you take, the more you leave behind. What am I? ", "FOOTSTEP", "F"));
        Questions.Add(new QuestionTemplate(6, "First you eat me, the you get eaten, what am I?", "FISHHOOK", "F"));
        NextButtonClick();
    ...
    {
        SceneManager.LoadScene("GameplayLevel5");
    ...
    void Update() // ===== function update every frame
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleScore_5 = ScoreFieldText.text = "" + Score;
            ...
            if (ramdomNumber == i)
            {
                if (onClick.currentword.Length == Questions[i].CorrectAnswer.Length)
                {
                    if (onClick.currentword == Questions[i].CorrectAnswer)
                    {
                        ...
                        if (QuestionCount == TotalQuestionPerRound)
                        {
							playersaveSystem.GetComponent<PlayerSaveSystem>().Riddlelevel5 = true;
                            NextButton.gameObject.SetActive(false);
                            Time.timeScale = 1f;
                            ProgressButton.gameObject.SetActive(true);
                            timeofgamestop = false;
    ...
    public void UpdateTimerUI() // ===== function the timer will show up overall answer after finish answering the question or go it wrong answered
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleTime_5 = Timeofgame.text = hourCount + "h:" + minuteCount + "m:" + (int)secondsCount + "s";
        if (secondsCount >= 60)
        {
            minuteCount++;
            secondsCount = 0;
