...
public class GameplayLevel2 : MonoBehaviour
{
    ...
    void Start() // ===== function Main Starting Program
    {
        ...
        Questions.Add(new QuestionTemplate(1, "I am white when i am dirty, and black when I am clean. What am I?", "BLACKBOARD", "B"));
        Questions.Add(new QuestionTemplate(2, "I can be cracked, I can be made, I can be told, I can be played", "JOKE", "J"));
        Questions.Add(new QuestionTemplate(3, "I'm white and used for cutting and grinding, when I'm damaged, humans usally remove me or fill me. for most animals I am useful tool", "TOOTH", "T"));
        Questions.Add(new QuestionTemplate(4, "What runs without legs?", "WATER", "W"));
        Questions.Add(new QuestionTemplate(5, "What word contains all of the twenty six letters?", "ALPHABET", "A"));
        Questions.Add(new QuestionTemplate(6, "Passed from father to son, And shared between brothers. Its importance is unquestioned, Though it is used more by others. ", "NAME", "N"));
        NextButtonClick();
    ...
    void Update() // ===== function update every frame
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleScore_2 = ScoreFieldText.text = "" + Score;
            ...
            if (ramdomNumber == i)
            {
                if (onClick.currentword.Length == Questions[i].CorrectAnswer.Length)
                {
                    if (onClick.currentword == Questions[i].CorrectAnswer)
                    {
                        ...
                        if (QuestionCount == TotalQuestionPerRound)
                        {
							playersaveSystem.GetComponent<PlayerSaveSystem>().Riddlelevel2 = true;
                            NextButton.gameObject.SetActive(false);
                            Time.timeScale = 1f;
                            ProgressButton.gameObject.SetActive(true);
                            timeofgamestop = false;
    ...
    public void UpdateTimerUI() // ===== function the timer will show up overall answer after finish answering the question or go it wrong answered
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleTime_2 = Timeofgame.text = hourCount + "h:" + minuteCount + "m:" + (int)secondsCount + "s";
        if (secondsCount >= 60)
        {
            minuteCount++;
            secondsCount = 0;
