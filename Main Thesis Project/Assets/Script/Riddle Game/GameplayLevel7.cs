...
public class GameplayLevel7 : MonoBehaviour
{
    ...
    void Start() // ===== function Main Starting Program
    {
        ...
        Questions.Add(new QuestionTemplate(1, "Four fingers and a thumb,  Yet flesh and blood,  I have none. What am I? ", "GLOVE", "G"));
        Questions.Add(new QuestionTemplate(2, "A slender body, a tiny eye, no matter what happens, I never cry. What am I?", "NEEDLE", "N"));
        Questions.Add(new QuestionTemplate(3, "Voiceless it cries, Wingless flutters, Toothless bites, Mouthless mutters. What is it?", "WIND", "W"));
        Questions.Add(new QuestionTemplate(4, "What can point in every direction but can't reach the destination by itself.", "FINGER", "F"));
        Questions.Add(new QuestionTemplate(5, "This is as light as a feather, yet no man can hold it for long. What am I?", "BREATHING", "B"));
        Questions.Add(new QuestionTemplate(6, "Poor people have it. Rich people need it. If you eat it you die. what is it?", "NOTHING", "N"));
        NextButtonClick();
    ...
    {
        SceneManager.LoadScene("GameplayLevel7");
    ...
    void Update() // ===== function update every frame
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleScore_7 = ScoreFieldText.text = "" + Score;
        ...
        for (int i = 0; i < Questions.Count; i++)
        {
            if (ramdomNumber == i)
            {
                if (onClick.currentword.Length == Questions[i].CorrectAnswer.Length)
                {
                    ...
                    {
						playersaveSystem.GetComponent<PlayerSaveSystem>().Riddlelevel7 = true;
    ...
    public void UpdateTimerUI() // ===== function the timer will show up overall answer after finish answering the question or go it wrong answered
    {
        ...
        playersaveSystem.GetComponent<PlayerSaveSystem>().RiddleTime_7 = Timeofgame.text = hourCount + "h:" + minuteCount + "m:" + (int)secondsCount + "s";
        if (secondsCount >= 60)
        {
            minuteCount++;
            secondsCount = 0;
